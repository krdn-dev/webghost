...
Да, веб‑сервер обязателен. WebGhost создаёт сайт и имитирует трафик, но сам не обрабатывает входящие HTTPS‑запросы. Веб‑сервер должен быть установлен и настроен отдельно, чтобы сайт был доступен из интернета. Если вы используете [NaiveProxy Manager](https://github.com/krdn-dev/naiveproxy-manager), веб‑сервер устанавливается автоматически.
